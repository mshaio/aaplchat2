...
def query_database(query_prompt: str) -> Dict[str, Any]:
    ...
    data = {"queries": [{"query": query_prompt, "top_k": 3, "include_metadata": True}]}
    print(query_prompt)
...
def call_chatgpt_api(user_question: str, chunks: List[str]) -> Dict[str, Any]:
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", # We can change to a different model here
        messages=messages,
        max_tokens=1024,
...
def ask(user_question: str) -> Dict[str, Any]:
    ...
    for result in chunks_response["results"]:
        for inner_result in result["results"]:
            ...
            sources.append(inner_result['metadata']['source_id'])
